...
function autoSaveLessonPlan(){
  ...
  _autoSaveLpTimer = setTimeout(async () => {
    try {
      ...
      const payload = {
        ...
        sessionStt: currentSessionStt,
        courseCode: p?.code || "",
        courseName: p?.name || "",
        totalHours: p?.totalHours || 0,
        lessonTitle: l?.title || "",
        periods: Number(document.getElementById("periods")?.value) || 0,
        minutes: Number(document.getElementById("minutes")?.value) || 0,
        lessonType: document.getElementById("lessonType")?.value || "theory",
        htmlContent: paper.innerHTML
      };
      
      const res = await API.saveLessonPlan(payload);
      if(res.data && res.data.id) currentLessonPlanId = res.data.id;
    } catch(e) {
      console.warn("Auto-save lesson plan error:", e);
    }
  }, 1200);
}
...
async function getAll(){
  try {
    ...
    console.warn("Using local programs fallback:", e);
    return programs.length ? programs : DEFAULT_PROGRAMS;
  }
}

async function addProgram(p){
  try {
    const res = await API.saveProgram(p);
...
async function ensureDefaults(){
  try {
    ...
    console.error("Error ensuring defaults:", e);
  }
}
...
async function getAIKey(){
  try {
    ...
    return res.data?.gemini_api_key || localStorage.getItem("giaoan_gemini_key") || "";
  } catch(e) {
    return localStorage.getItem("giaoan_gemini_key") || "";
  }
}
...
async function getAIModel(){
  try {
    const res = await API.getSettings();
    return res.data?.gemini_model || localStorage.getItem("giaoan_gemini_model") || "gemini-2.5-flash";
  } catch(e) {
    return localStorage.getItem("giaoan_gemini_model") || "gemini-2.5-flash";
  }
}

async function saveAISettings(){
  const k = $("#aiKey").value.trim();
  const m = $("#aiModel").value.trim() || "gemini-2.5-flash";
  localStorage.setItem("giaoan_gemini_key", k);
  localStorage.setItem("giaoan_gemini_model", m);
  try {
    await API.saveSettings({ gemini_api_key: k, gemini_model: m });
    showToast("Đã lưu cấu hình AI vào máy chủ!", "success");
  } catch(e) {
    showToast("Đã lưu cấu hình AI cục bộ", "info");
  }
  closeModal();
}
...
async function loadAISettings(){
  ...
  try {
    const res = await API.getSettings();
    if(res.data) {
      k = res.data.gemini_api_key || "";
      m = res.data.gemini_model || "gemini-2.5-flash";
    }
  } catch(e) {}
  if(!k) k = localStorage.getItem("giaoan_gemini_key") || "";
  if(k && $("#aiKey")) $("#aiKey").value = k;
  if(m && $("#aiModel")) $("#aiModel").value = m;
}
...
async function generateWithGemini(key, model, prompt){
  try {
    const res = await API.generateAI(prompt, model, key);
    if(res.text) return res.text;
    throw new Error(res.error || "Không nhận được phản hồi từ AI");
  } catch(err) {
    // Direct fallback if proxy fails
    console.warn("Backend AI proxy error, attempting direct client call:", err);
...
async function exportPdf(){
  ...
  // Loại bỏ toàn bộ thành phần giao diện không thuộc giáo án.
  ...
  // Tạo container riêng đúng khổ A4.
  ...
  // Bỏ khung xem trước, bóng đổ và padding màn hình.
  ...
  const opt={
    margin:[20,15,20,20], // mm: trên, phải, dưới, trái
    filename,
...
async function receiveScheduleSession(data){
  ...
  // BẮT BUỘC: Loại giáo án được xác định từ cơ cấu tiết của chính buổi trong Sổ đầu bài.
  // Người dùng không tự chọn loại giáo án tại Bước 2.
  ...
  if(pld.courseMode==="theory" && (ltCount>0||ktCount>0)){gaType="theory";gaReason="Môn Lý thuyết → dùng Giáo án Lý thuyết.";
  }else if(pld.courseMode==="practice" && (thCount>0||ktCount>0)){gaType="practice";gaReason="Môn Thực hành → dùng Giáo án Thực hành.";
  }else if(ltCount>0 && thCount>0){
    gaType="integrated";
    ...
    // Nếu buổi chỉ có Kiểm tra, giữ logic theo dữ liệu Sổ đầu bài:
    // mốc kiểm tra không có LT/TH đi kèm được xử lý như một buổi kiểm tra độc lập.
    gaType=pld.courseMode==="theory"?"theory":"practice";
    gaReason=pld.courseMode==="theory"?`Buổi học có ${ktCount} tiết Kiểm tra Lý thuyết → Giáo án Lý thuyết.`:`Buổi học có ${ktCount} tiết Kiểm tra Thực hành → Giáo án Thực hành.`;
  }else{
    if(info){
      info.className="status bad";
  ...
  // BẮT BUỘC: Số tiết lấy trực tiếp từ đúng buổi trong Sổ đầu bài.
  // Không cho chọn, không giữ danh sách 1–12 tiết, không dùng giá trị mặc định.
  ...
  // BẮT BUỘC: Tổng thời gian lấy từ Sổ đầu bài Bước 1.
  // Quy đổi theo Skill: 1 tiết = 45 phút.
  ...
  // Bước 2 chỉ sử dụng dữ liệu nhận từ Bước 1.
  // Xóa mọi dữ liệu môn mặc định/phiên trước khỏi giao diện Bước 2 để tránh chọn nhầm.
  programs=[];

  // Tạo chương trình tạm theo đúng nội dung của buổi Sổ đầu bài.
  ...
  // Không ghép tên nhiều bài bằng dấu "+".
  // Giữ nguyên từng nhóm Tên bài -> đề mục như Sổ đầu bài.
  const tempProgram={
    id:tempId,
    code:sourceCode,
    name:sourceName,
    ...
    scheduleDate:String(pld.date||"").trim(),
    scheduleAssessment:{
      ktCount:ktCount,
      type:pld.assessmentType||"",
      label:pld.assessmentType?(`Kiểm tra ${pld.assessmentType}`):"Kiểm tra"
    },
    lessons:[{
      title:fallbackTitle,
      hours:periods,
  ...
  // Gắn duy nhất môn/buổi nhận từ Bước 1 vào Bước 2.
  ...
  // BẮT BUỘC: Bài / Chương tại Bước 2 lấy đúng từ Sổ đầu bài và không cho chọn lại.
  ...
  // Tên bài học trước lấy từ TT liền trước trong Sổ đầu bài,
  // gồm TÊN BÀI + toàn bộ đề mục/tiểu mục thực tế đã dạy trong ngày đó.
  // Nếu đang soạn TT 1 thì để trống.
  ...
  // Giáo án số luôn lấy trực tiếp từ STT của Sổ đầu bài.
  $("#gaNumber").value=String(pld.scheduleTT||"");
  $("#gaNumber").readOnly=true;
  $("#gaNumber").title="Giáo án số được lấy tự động từ STT của Sổ đầu bài.";
  ...
  // Cuộn đến khu vực chọn nội dung để người dùng có thể điều chỉnh nếu muốn.
...
async function init(){
  await openDB();
  // Bước 2 KHÔNG hiển thị sẵn môn học. Chỉ nạp dữ liệu khi nhận một buổi từ Sổ đầu bài ở Bước 1.
  programs=[];

  // Bước 2 không có Số tiết mặc định.
  // Số tiết chỉ được nhận từ đúng buổi học trong Sổ đầu bài ở Bước 1.
...
$("#modalClose").onclick=closeModal;$("#modal").onclick=e=>{if(e.target.id==="modal")closeModal()};
  $("#btnBackup").onclick=backup;$("#btnRestore").onclick=()=>$("#restoreFile").click();
